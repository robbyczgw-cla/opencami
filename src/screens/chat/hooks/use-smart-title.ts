...
/**
 * Hook for generating smart session titles via OpenClaw Gateway.
 * Falls back to heuristic generation if the gateway is unavailable.
 */
export function useSmartTitle(): UseSmartTitleResult {
    ...
    async (message: string): Promise<GenerateTitleResult> => {
      ...
      try {
        const res = await fetch('/api/llm-features', {
          ...
          body: JSON.stringify({ action: 'title', message }),
        ...
        return { title, source, error: data.error }
      } catch (err) {
  ...
  return { generateTitle, isGenerating, lastTitle, lastSource }
}
...
/**
 * LLM titles are always enabled (via OpenClaw Gateway)
 * @deprecated Use useSmartTitle directly
 */
